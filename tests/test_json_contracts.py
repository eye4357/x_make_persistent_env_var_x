...
def test_main_json_persist_values_success(
    ...
    monkeypatch.setenv("API_TOKEN", "session-secret")
    ...
    assert summary_obj.get("action") == "persist-values"
    assert summary_obj.get("tokens_modified") == 2
    ...
    assert isinstance(provided_obj, dict)
